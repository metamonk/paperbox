...
export class FabricCanvasManager {
  ...
  initialize(config: FabricCanvasConfig): fabric.Canvas {
    // TODO Day 1: Create canvas element
    this.containerElement = document.createElement('canvas');
    this.containerElement.id = config.containerId;

    // TODO Day 1: Initialize Fabric.js canvas
    this.canvas = new fabric.Canvas(this.containerElement, {
      ...
      skipTargetFind: false,    // Enable object selection
      enableRetinaScaling: true, // HiDPI support
    });

    // TODO Day 2: Setup event listeners
   ...
   */
  toCanvasObject(fabricObj: fabric.Object): Partial<CanvasObject> {
   ...
   */
  findObjectById(id: string): fabric.Object | null {
    if (!this.canvas) return null;

    const objects = this.canvas.getObjects();
    return objects.find(obj => obj.data?.id === id) || null;
  }
